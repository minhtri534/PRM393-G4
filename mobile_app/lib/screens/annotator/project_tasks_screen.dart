...
class AnnotatorProjectTasksScreen extends StatefulWidget {
  ...
  const AnnotatorProjectTasksScreen({super.key, required this.project});

  @override
  State<AnnotatorProjectTasksScreen> createState() =>
      _AnnotatorProjectTasksScreenState();
}

class _AnnotatorProjectTasksScreenState
    extends State<AnnotatorProjectTasksScreen> {
  int _tab = 0;
        ...
        builder: (context, provider, _) {
          ...
          return Padding(
            ...
            child: Column(
              ...
              children: [
                DlssPageHeader(
                  title: WorkflowStrings.annotatorTasksTitle,
                  subtitle: widget.project.guideline?.trim().isNotEmpty == true
                      ? widget.project.guideline!
                      : WorkflowStrings.annotatorTasksSubtitleFallback,
                ),
...
class _ProjectTaskListBody extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    if (provider.listState == AnnotatorLoadState.error &&
      ...
      return error_widget.ErrorWidget(
        ...
        onRetry: () => provider.fetchTasks(projectId: projectId),
      );
    }
    ...
    if (filteredTasks.isEmpty) {
      return DlssEmptyState(
        ...
        message: showTodoTab
            ? WorkflowStrings.annotatorNoProjectTodoTasks
            : WorkflowStrings.annotatorNoProjectDoneTasks,
      );
    }

    return RefreshIndicator(
      onRefresh: () => provider.fetchTasks(projectId: projectId),
  ...
  Future<void> _startLabeling(
    ...
    if (result == true && context.mounted) {
      provider.fetchTasks(projectId: projectId);
    }
  }
}
